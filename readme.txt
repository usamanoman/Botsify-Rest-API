...
Want to get your site's posts? Simply send a `GET` request to `/wp-json/wp/botsify/posts`. Update user with ID 4? Send a `PUT` request to `/wp-json/wp/botsify/users/4`. Get all posts with the search term "awesome"? `GET /wp-json/wp/botsify/posts?filter[s]=awesome`. It's that easy.
...
* Adds `roles` param for `GET /wp/botsify/users`.
...
* Ensures `GET /wp/botsify/categories` with out of bounds offset doesn't return results.
...
* BREAKING CHANGE: Removes `post_*` query param support for `GET /wp/botsify/comments`.

  The proper pattern is to use `GET /wp/botsify/posts` to fetch the post IDs to limit the request to.
...
* Introduces `media_type` and `mime_type` params for `GET /wp/botsify/media`.
...
* Validates `GET /wp/botsify/comments` private query params.
...
* Adds `trash` status to `GET /wp/botsify/statuses`.
...
* BREAKING CHANGE: Moves Post->Term relations to the Post Resource
  ...
  Previously, a client would fetch a Post's Tags with `GET /wp/botsify/posts/<id>/tags`.

  In Beta 11, an array of term ids is included on the Post resource.

  The collection of terms for a Post can be fetched with `GET /wp/botsify/tags?post=<id>`.
...
* Renames `GET /wp/botsify/comments` `user` param to `author` to match resource attribute.
...
* Adds support for `GET /wp/botsify/pages parent=1,2,3`.
...
* Add `parent_exclude` param to `GET /wp/botsify/posts`.
...
* Forces `GET /wp/botsify/media` to be limited to `'status' => [ inherit, private, trash ]`
...
* Requires `force` argument for `DELETE /wp/botsify/<taxonomy>/<id>`.
...
* Abstracts `POST /wp/botsify/media` permissions check.
...
* Adds `rest_comment_query` for filtering `GET /wp/botsify/comments`.
...
* In `GET /wp/botsify/<taxonomy>`, drops support for `orderby=>term_id`.
...
* BREAKING CHANGE: Introduces `edit` context for `wp/botsify/types` and `wp/botsify/taxonomies`.
...
* Adds `include` param to `/wp/botsify/posts`, `/wp/botsify/users`, `/wp/botsify/<taxonomy>` and `/wp/botsify/comments`.
...
* Ensures `GET /wp/botsify/posts` respects `order` and `orderby` params.
...
* BREAKING CHANGE: Move tags and categories to top-level endpoints.
  ...
  Tags are now accessible at `/wp/botsify/tags`, and categories accessible at `/wp/botsify/categories`. Post terms reside at `/wp/botsify/posts/<id>/tags` and `/wp/botsify/<id>/categories`.

  (props @danielbachhuber, [#1802](https://github.com/WP-API/WP-API/pull/1802))

* BREAKING CHANGE: Return object for requests to `/wp/botsify/taxonomies`.

  This is consistent with `/wp/botsify/types` and `/wp/botsify/statuses`.
...
* Add missing 'wp/botsify' in Location Response header when creating new Post Meta.
...
* Embed links inside items in a collection.
  ...
  No longer! You can now request a collection with embeds enabled (try `/wp/botsify/posts?_embed`). This will embed links inside each item, allowing you to build interface items much easier (for example, post archive pages can get featured image data at the same time).
...
* Respect `rest_base` for taxonomies.
  ...
  **⚠️ This breaks backwards compatibility** by changing the `/wp/botsify/posts/{id}/terms/post_tag` endpoint to `/wp/botsify/posts/{id}/tag`.
